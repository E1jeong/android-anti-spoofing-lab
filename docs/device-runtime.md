...
- Normal inference rejects an RGB face box touching an image edge or moving faster than 0.8 face widths per second; it resumes after one stable tracking frame. On rejection, clear the prior classification while retaining the yellow face box, and discard any already-running result from the earlier motion generation. Do not apply this gate to collection.
