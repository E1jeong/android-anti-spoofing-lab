...
- On `feat/mediapipe-face-detector-toggle`, FaceMe and MediaPipe Face Detector are initialized together and the UI can select either for tracking and calibration. MediaPipe runs on CPU in the existing tracking executor; do not invoke it from camera callbacks or enable a second accelerator path before target-device measurement. Its detected box expands 25% above and 5% below the raw height, with image-bound clamping; the model spec's crop margin remains separate. The branch's `live` collection keeps the FaceMe HIGH/MEDIUM quality contract, so it must reject live collection while MediaPipe is selected.
