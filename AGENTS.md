...
- **Current deployment supports float and full INT8 models.** Each spec chooses `cpu` or `nnapi`. A CPU spec uses CPU/XNNPACK; an NNAPI setup or model warmup failure rejects that manifest slot instead of falling back to CPU. This no-fallback policy is intentional for the NPU evaluation tool. Do not report acceleration until model warmup, the on-device backend label, logcat, and latency are checked.
- `Backend NNAPI` shows the requested interpreter path after successful allocation and warmup, not proof that every operation executed on the NPU.
...
- Since 2026-07-16, detailed timing stays out of the on-screen diagnostics: `MainActivity:I` logs a rolling 120-sample P50/P95 summary for model preprocess, TFLite invoke, inference queue wait, and tracking-queue-to-result time every 30 inferences, plus capture-save P50/P95 every 10 attempted saves. Use `adb logcat -s MainActivity:I` to collect the baseline.
- Live save candidates now reuse one FaceMe extraction for tracking and quality data. Capture I/O owns a detached RGB/IR frame pair and writes full/crop BMPs directly from the source bitmaps with a reusable 16-row stripe buffer and `BufferedOutputStream`, eliminating two full-frame copies and two crop bitmap creations per attempted save while preserving 24-bit bottom-up output. The IR diagnostic preview reuses one ARGB bitmap/Canvas while dimensions remain stable but still performs its throttled full-frame copy. These changes passed 17 JVM tests and Java compilation; target-device quality acceptance, BMP/cancel regression, camera-pool pressure, latency, and GC still require verification before claiming a speedup.
