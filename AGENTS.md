...
- Capture BMP output now uses a reusable 16-row stripe buffer and `BufferedOutputStream`, preserving 24-bit bottom-up BMP output. The IR diagnostic preview reuses one ARGB bitmap/Canvas while dimensions remain stable; it still performs the full-frame copy. Both changes passed JVM tests and Java compilation but still require target-device capture/preview regression and latency/GC measurement before claiming a speedup.
